...
export const createPost = async({ content, user, imageFile }) => {
    if (imageFile) {
        const { data: imageData, error: imageError } = await supabase.storage.from('images').upload(getUser().email + '/' + Date.now(), imageFile, {
            cacheControl: '3600',
            upsert: false
        });
        ...
        const { data, error } = await supabase.from('posts').insert({content, user, image: imageData.Key})
        return { data, error }
    } else {
